...
public class FirebaseLikeDataSource implements LikeDataSource{
    ...
    public ObservableTask<List<Like>> getLikes(final String photoId) {
        return new ObservableTask<List<Like>>() {
            @Override
            public void run(final Subscriber<List<Like>> result) {
                final DatabaseReference likesRef = FirebaseDatabase.getInstance().getReference("Photos")
                        .child(photoId)
                        .child("Likes");
    ...
    @Override
    public ObservableTask<Boolean> toggleLike(final Like like) {
    ...
    @Override
    public ObservableTask<Pair<Like, Boolean>> addLikeNotifier(final String photoId) {
        return new ObservableTask<Pair<Like, Boolean>>() {
            @Override
            public void run(final Subscriber<Pair<Like, Boolean>> result) {
                ...
                ChangeEventListener<Long> listener = new ChangeEventListener<Long>(likesRef, Long.class) {
                    @Override
                    protected void onChildAdded(String userId, Long data) {
                    ...
                    @Override
                    protected void onChildRemoved(String userId, Long data) {
                        Like like = Like.Builder()
                                .photoId(photoId)
                                .userId(userId)
                                .build();
    ...
    @Override
    public ObservableTask<Boolean> removeLikeNotifier(final String photoID) {
        return new ObservableTask<Boolean>() {
            @Override
